...
G = nx.read_graphml(f'{par.DATA_PATH}random_graph2.graphml')

# -> PLOT graph in /plots 
fig, ax = plt.subplots()
pos = plotting.plot_graph('random_graph2')
fig.tight_layout()
fig.savefig(f"../plots/general_network/graph2.pdf")
...
# training.train(G_ml, training_steps=training_steps, weight_type='length', delta_weight = 1e-3, learning_rate=1e-5)
G_ml = G.copy(as_view=False)  
